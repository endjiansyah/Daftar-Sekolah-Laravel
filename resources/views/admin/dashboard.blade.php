...
<head>
    ...
    <style>
        ...
        .navbar { background-color: #0f172a; padding: 1rem 0; }
        .table-card { border: none; border-radius: 12px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
        .modal-content { border-radius: 20px; border: none; box-shadow: 0 25px 50px -12px rgb(0 0 0 / 0.25); }
        ...
        .section-tag { 
            ...
            border-radius: 6px; margin: 1.5rem 0 1rem 0; text-transform: uppercase;
        }
        .detail-item { 
            display: flex; padding: 12px 0; border-bottom: 1px solid #f1f5f9; align-items: flex-start;
        }
        .detail-label { width: 35%; color: #64748b; font-size: 0.85rem; font-weight: 500; }
        .detail-value { width: 65%; color: #1e293b; font-size: 0.9rem; font-weight: 600; word-break: break-word; }
        .badge-status { font-size: 0.75rem; padding: 6px 12px; border-radius: 8px; font-weight: 600; }

        /* Memastikan semua tombol aksi punya tinggi yang sama agar selaras */
        .btn-action { height: 34px; display: inline-flex; align-items: center; justify-content: center; }
    </style>
</head>
<body>
                            ...
                            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body px-4 pb-4">
                                        ...
                                        <div class="detail-item"><div class="detail-label">Nama Lengkap</div><div class="detail-value">{{ $student->full_name }}</div></div>
                                        <div class="detail-item"><div class="detail-label">Email Akun</div><div class="detail-value text-primary">{{ $student->email }}</div></div>
                                        <div class="detail-item"><div class="detail-label">NISN</div><div class="detail-value">{{ $student->nisn ?? '-' }}</div></div>
                                        <div class="detail-item"><div class="detail-label">No. WhatsApp</div><div class="detail-value">{{ $student->phone_number ?? '-' }}</div></div>
                                        <div class="detail-item"><div class="detail-label">Jenis Kelamin</div><div class="detail-value">{{ $student->gender == 'L' ? 'Laki-laki' : ($student->gender == 'P' ? 'Perempuan' : '-') }}</div></div>
                                        <div class="detail-item"><div class="detail-label">Tempat Lahir</div><div class="detail-value">{{ $student->birthCity->name ?? '-' }}</div></div>
                                        <div class="detail-item"><div class="detail-label">Tanggal Lahir</div><div class="detail-value">{{ $student->dob ? $student->dob->format('d F Y') : '-' }}</div></div>
                                        <div class="detail-item"><div class="detail-label">Alamat Lengkap</div><div class="detail-value">{{ $student->address ?? '-' }}</div></div>

                                        <div class="section-tag">II. Data Orang Tua / Wali</div>
                                        @if($student->parentDetail)
                                            <div class="detail-item"><div class="detail-label">Nama Orang Tua</div><div class="detail-value">{{ $student->parentDetail->parent_name }}</div></div>
                                            <div class="detail-item"><div class="detail-label">Hubungan</div><div class="detail-value">{{ $student->parentDetail->relationship }}</div></div>
                                            <div class="detail-item"><div class="detail-label">WhatsApp Ortu</div><div class="detail-value text-success">{{ $student->parentDetail->parent_phone }}</div></div>
                                            <div class="detail-item"><div class="detail-label">Email Ortu</div><div class="detail-value">{{ $student->parentDetail->parent_email ?? '-' }}</div></div>
                                        @else
                                            <div class="p-3 bg-light rounded text-danger small fw-bold">Data orang tua belum dilengkapi.</div>
                                        @endif

                                        <div class="section-tag">III. Informasi Sekolah Asal</div>
                                        @if($student->schoolDetail)
                                            <div class="detail-item"><div class="detail-label">Nama Sekolah</div><div class="detail-value">{{ $student->schoolDetail->school_name }}</div></div>
                                            <div class="detail-item"><div class="detail-label">Kota Sekolah</div><div class="detail-value">{{ $student->schoolDetail->city->name ?? '-' }}</div></div>
                                            <div class="detail-item"><div class="detail-label">Tahun Lulus</div><div class="detail-value">{{ $student->schoolDetail->graduation_year }}</div></div>
                                            <div class="detail-item" style="border-bottom:none;"><div class="detail-label">Alamat Sekolah</div><div class="detail-value">{{ $student->schoolDetail->school_address ?? '-' }}</div></div>
                                        @else
                                            <div class="p-3 bg-light rounded text-danger small fw-bold">Data sekolah asal belum dilengkapi.</div>
                                        @endif
                                    </div>
                                            ...
                                            @if($currentStatus == 'DAFTAR')
                                                ...
                                                <form action="{{ route('admin.verify', [$student->id, 'TERVERIFIKASI']) }}" method="POST" class="m-0">
                                                    ...
                                                    <button type="submit" class="btn btn-success px-5 fw-bold shadow-sm btn-action" {{ !$isComplete ? 'disabled' : '' }}>
                                                        {{ $isComplete ? 'Terima & Verifikasi' : 'Data Belum Lengkap' }}
                                                    </button>
                                                </form>
                                            @else
                                                <form action="{{ route('admin.verify', [$student->id, 'DAFTAR']) }}" method="POST" class="m-0">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="btn btn-secondary fw-bold px-4 btn-action" onclick="return confirm('Kembalikan status ke Daftar?')">
                                                        <i class="bi bi-arrow-counterclockwise me-1"></i> Kembalikan ke Daftar
                                                    </button>
                                                </form>
                                            @endif
                                        </div>
